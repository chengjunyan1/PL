...
class MobileNetV3(nn.Module):
    ...
    def _forward_impl(self, x: Tensor, pred=False) -> Tensor:
        x = self.features(x)

        x = self.avgpool(x)
        x = torch.flatten(x, 1)

        if pred: x = self.classifier(x)

        return x

    def forward(self, x: Tensor, pred=False) -> Tensor:
        return self._forward_impl(x, pred)
